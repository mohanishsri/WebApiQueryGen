...
namespace WebAPI.Models
{
    public class dbAddRecipe
    {
        ...
        public IEnumerable<attributecolval> GetColNamesForRecipe()
        {
            IList<attributecolval> lstreceipe = new List<attributecolval>();

            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("Get_ColumnNameAsTable"))
                {
                    cmd.Connection = con;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@tablename", "GIRFT_NCIP_RnD_BaseComponent");
